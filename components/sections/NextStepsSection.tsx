...
//
// The journey is a plain grid, with no positioned offsets to keep in step:
...
// The connecting line is not one long absolute element: each ring is followed, in the same flex row,
// by the segment that leads to the next ring (see journey.css), so it always meets the rings.
//
            ...
            <ol className="mx-auto grid max-w-3xl auto-rows-auto [--jn-gap:2rem] lg:max-w-none lg:grid-cols-4 lg:gap-x-[var(--jn-gap)] xl:[--jn-gap:3rem]">
              {JOURNEY_STEPS.map((step, index) => {
                ...
                return (
                  ...
                  >
                    <span className="col-start-2 row-start-1 h-5 text-[12px] leading-5 font-semibold tracking-[0.14em] text-accent lg:col-start-1 lg:mb-5">
                      {step.number}
                    </span>

                    {/* The ring, and the line segment that leads on to the next ring */}
                    <div className="col-start-1 row-span-2 row-start-1 flex flex-col items-center lg:row-span-1 lg:row-start-2 lg:flex-row">
                      <div
                      ...
                      {!isLast && (
                        <div
                          aria-hidden="true"
                          className={`jn-seg-${stage} relative w-px flex-1 bg-white/15 lg:mr-[calc(var(--jn-gap)*-1)] lg:h-px lg:w-auto`}
                        >
